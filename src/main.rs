...
";
    ...
    //
    // However, there's rare cases where, during select, we called table_start, which then
    // called table_find to find our first element. Since the element exist, instead of returning
    // the right cursor, we return error, which our impl fallback to our previous incorrect
